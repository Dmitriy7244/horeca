...
async def send_change_ad_keyboard(_):
...
async def send_change_company_info_keyboard(_):
...
async def send_change_vacancy_keyboard(_, button: dict):
    ...
    await storage.update_data({CreateAdKeys.CURRENT_VACANCY_NUM: vacancy_num})
...
async def send_change_extra_info_keyboard(_):
