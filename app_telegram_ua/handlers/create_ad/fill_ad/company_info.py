...
async def process_regional_city(_):
    regional_city = message.text
    ...
    await send_next_or_ad_preview_message(
        message.answer(texts.choose_city, reply_markup=kb.Cities(regional_city)),
    )


@dp.message_handler(button=kb.Cities.BUTTONS, state=Conv.city)
async def process_city(_):
    city = message.text
    ...
    await send_next_or_ad_preview_message(
        message.answer(texts.enter_company_type, reply_markup=ReplyKeyboardRemove()),
    )


@dp.message_handler(state=Conv.type)
async def process_company_type(_):
...
async def process_company_name(_):
    name = message.text
...
async def process_company_address(_):
