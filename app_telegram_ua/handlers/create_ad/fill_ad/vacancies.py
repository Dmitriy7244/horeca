...
async def process_vacancies_num(_, button: dict):
    ...
    await message.answer(texts.enter_vac_title, reply_markup=ReplyKeyboardRemove())


@dp.message_handler(state=Conv.title)
async def process_title(_):
    title = message.text
...
async def process_work_experience(_):
...
async def process_working_hours(_):
